...
function topbollywood() 
{
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) 
    {
      ...
      for (i in myObj) 
      {
        toptenbollywood(astr.concat(myObj[i].album_loc),myObj[i].name);
      }
    }
  };
  xhttp.open("GET", "../php/Load data php/topbollywood.php?q=", true);
...
function tophollywood()
{
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) 
    {
      ...
      for (i in myObj) 
      {
        toptenhollywood(astr.concat(myObj[i].album_loc),myObj[i].name);
      }
    }
  };
...
function topglobal() 
{
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) 
    {
      ...
      for (i in myObj) 
      {
        toptenglobal(astr.concat(myObj[i].album_loc),myObj[i].name);
      }
    }
  };
...
function justarrived() 
{
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) 
    {
      ...
      for (i in myObj) 
      {
        justtarrived(astr.concat(myObj[i].album_loc),myObj[i].name);
      }
    }
  };
